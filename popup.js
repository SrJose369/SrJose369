...
var youOtwitch = null;
/**
 * Variable que controla si mostrar o no los consloe.log
 */
const showMsj = true;
 ...
 */
const willy = new Streamer("Willy", "11", "Viendo", "22", "WFinish 2021-06-05 20_18_39", "Y");
/**
 * Informacion sobre vege 90 carpeta principal
 */
const vege = new Streamer("Vege", "123", "Viendo", "132", "VFinish 2021-06-05 20_18_39", "Y");
/**
 ...
 */
function setTextCanal(text) {
	document.getElementById("canal").innerText = text;
}
function continuarBok() {
	if (getData(true) === false) {
		setTextCanal("Err :(");
		return;
	}
				...
				for (i = 0; i < v.length; i+=1) {
					if (youOtwitch !== "T") {
						...
						if (willyfound === true && vegefound === true) {
							mensaje("Se llego al final Youtube i "+i+" v.length "+v.length);
							break;
						}
					}
					if (youOtwitch !== "Y") {
						...
						if (rubiusfound === true && lelyfound === true) {
							mensaje("Se llego al final Twitch i "+i+" v.length "+v.length);
							break;
						}
					}
				}
					...
					if (bokNode.title === stre.nombre && bokNode.url === undefined) {
						...
						for (j = 0; j < child.length; j+=1) {
							...
							if (contFound === true && finiFound === true) {
								mensaje("Se llego al final i "+i+" v.lenght "+v.length);
								break;
							}
						}
						bol = true;
...
function getData(cont_O_finish) {
	...
	let aux;
	const ab = formatTi(xtie);
	if (ab === "Err") {
		mensaje("Error en format tiempo");
		return false;
	}
	if (cont_O_finish) {
		aux = actDate() + " " + ab + " " + xtit;
...
function mensaje(ms) {
	//let va = "at mensaje (popup.js:679)";
	//let st = Error().stack;
	//st = st.substring(va.length);
	//console.log(st);
	if (showMsj === true) {
		console.log(ms);
	}
...
function formatTi(tiem) {
	// 00_13_23
	if (checkNull(tiem)) {
		mensaje("Err tiem null");
		return "Err";
	}
	if (checkNull(tiem.length)) {
		mensaje("Err tiem length null");
		return "ErrL";
	}
	if (tiem.length < 4) {
		mensaje("no deberia tener menos de 4 length ("+tiem+")");
		return "Err";
	}
	if (tiem.length > 8) {
		mensaje("no deberia tener mas de 8 length ("+tiem+")");
		return tiem;
	}
	const x = tiem.replaceAll(":", "_");
	let i;
	let c = 0;
	for (i = tiem.length-1; i >= 0; i-=1) {
		if (c === 2 || c === 5) {
			if (tiem[i] !== ":") {
				mensaje("Se esperaba : tiem["+c+"] "+tiem[i]);
				mensaje(tiem);
				return "Err";
			}
		} else {
			if (checkSoloDigitos(tiem[i]) === false) {
				mensaje("Err tiem["+c+"] "+tiem[i]);
				mensaje(tiem);
				return "Err";
			}
		}
...
function checkSoloDigitos(text) {
